  ...
  const [searchResults, setSearchResults] = useState(data); // data provided
    ...
    const handleSearch = (searchTerm) => {
      const filteredData = data.filter(item => {
        const subjectTitleMatch = item.subjectTitle && typeof searchTerm === 'string' && item.subjectTitle.toLowerCase().includes(searchTerm.toLowerCase());
        const researchersMatch = item.researchers && typeof searchTerm === 'string' && item.researchers.toLowerCase().includes(searchTerm.toLowerCase());
        const typeMatch = item.type && typeof searchTerm === 'string' && item.type.toLowerCase().includes(searchTerm.toLowerCase());
        const AYMatch = item.AY && typeof searchTerm === 'string' && item.AY.toLowerCase().includes(searchTerm.toLowerCase());

        return subjectTitleMatch || researchersMatch || typeMatch || AYMatch;
...
return (
  <div>
    ...
    <article className='grid grid-cols-1 gap-4 m-6 lg:grid-cols-3 md:grid-cols-2 xl:grid-cols-3'>
